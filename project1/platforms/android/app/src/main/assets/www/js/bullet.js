...
function bullet(pointX,pointY,playerX,playerY){  
   ...
   this.sparks=[];
	...
	this.update= function(){
      if(this.animateExplosion==false){
            ...
            this.boxX=this.currentX-this.radius;
            this.boxY=this.currentY-this.radius;
            this.draw();
            this.checkOutside();
      }
      else{
	...
	this.draw = function(){  
         ...
         b_context.arc(0,0,this.radius,0,2*Math.PI);
         b_context.fillStyle="grey";
         b_context.fill();
         b_context.stroke();
         ...
         // b_context.rect( -this.radius,-this.radius,this.cir,this.cir); 
         // b_context.stroke();
   ...
   this.collide=function(){
      ...
      for(this.i=0;this.i<100;this.i++) {
         ...
         this.sparks.push(new Particle(this.currentX,this.currentY));
   ...
   this.drawExplosion=function(){
         ...
         for(this.i=0;this.i<this.sparks.length;this.i++) {

             this.sparks[this.i].update();
             if(this.sparks[this.i].destroy==true){
               delete this.sparks[this.i];
               this.sparks.splice(this.i,1);
